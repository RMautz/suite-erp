...
- **Conteos contractuales finales:** pgTAP **435 + 23 = 458 asserts en 21 archivos** (`whatsapp.test.sql` con `plan(23)`); unit **229 + 30 = 259** (14 en `@suite/whatsapp`, 16 en `@suite/bot` — los 2 extra son del fix de la review final: historial saneado y 5 rondas agotadas); 3 builds verdes.
...
4. `ClaudeMotor` no se ejercita contra la API viva (sin `ANTHROPIC_API_KEY` por diseño): queda probado con fetch mock sobre el contrato Messages API (tool_use → tool_result → texto). CERRADO en la review final: la API tolera turnos consecutivos del mismo rol pero rechaza un PRIMER turno assistant — y tras vincular el primer mensaje del log es siempre el código (saliente); el motor ahora descarta los turnos previos al primer entrante (test dedicado).
4b. Dedup de message-id de Meta y procesamiento de lotes (`messages[1..]`) quedan como PRERREQUISITO del modo `cloud` (credencial #15): hoy solo se procesa `messages[0]` y una redelivery duplicaría respuesta y log. Sin impacto en mock.
